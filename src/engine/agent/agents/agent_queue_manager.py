...
from src.engine.agent.entities.queue_entity import QueueEvent, AgentThought
...
class AgentQueueManager:
    ...
    def _is_stopped(self, task_id: UUID) -> bool:
        ...
        result = redis_client.redis.get(task_stopped_cache_key)
    ...
    def set_stop_flag(cls, task_id: UUID, invoke_from: InvokeFrom, user_id: UUID) -> None:
        ...
        result = redis_client.redis.get(cls.generate_task_belong_cache_key(task_id))
        if not result:
            return
        ...
        redis_client.redis.setex(stopped_cache_key, 600, 1)
