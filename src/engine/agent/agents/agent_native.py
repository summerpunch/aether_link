...
from src.engine.agent.entities.agent_entity import (
    AgentState,
...
from src.engine.agent.base_agent import BaseAgent


class AgentNative(BaseAgent):
    """基于函数/工具调用的智能体"""
    ...
    def _build_agent(self) -> CompiledStateGraph:
        ...
        graph.add_node("long_memory_node", self._long_memory_node)
        graph.add_node("llm_node", self._llm_node)
        graph.add_node("tools_node", self._tools_node)

        graph.set_entry_point("pre_node")
        graph.add_conditional_edges("pre_node", self._pre_condition)
        graph.add_edge("long_memory_node", "llm_node")
        graph.add_conditional_edges("llm_node", self._tools_condition)
    ...
    def _pre_node(self, state: AgentState) -> AgentState:
    ...
    def _long_memory_node(self, state: AgentState) -> AgentState:
        """长期记忆召回节点"""
        ...
        if isinstance(history, list) and len(history) > 0:
            ...
            if len(history) % 2 != 0:
                self.agent_queue_manager.publish_error(state.get("task_id","111"), "智能体历史消息列表格式错误")
                logging.exception(
    ...
    def _llm_node(self, state: AgentState) -> AgentState:
        ...
        #
        #             self.agent_queue_manager.publish(state.get("task_id","222"), AgentThought(
        #                 id=id,
        #                 task_id=state.get("task_id","222"),
        #                 event=QueueEvent.AGENT_MESSAGE,
        #                 thought=content,
        #                 message=messages_to_dict(state["messages"]),
        #                 answer=content,
    ...
    @classmethod
    def _tools_condition(cls, state: AgentState) -> Literal["tools_node", "__end__"]:
    ...
    @classmethod
    def _pre_condition(cls, state: AgentState) -> Literal["long_memory_node", "__end__"]:
